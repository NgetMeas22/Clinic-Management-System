...
const Payments = () => {
    ...
    const [pendingDelete, setPendingDelete] = useState(null); // payment object
    const [deleting, setDeleting] = useState(false);

    const canCreate = can(user, "payments", "create");
    const canDelete = can(user, "payments", "delete");

    // Reusable fetch — also called after create/delete to refresh the list.
    // Not used as the *effect* itself (see below), so callers always get the
    // latest result regardless of race conditions from rapid filter changes.
    ...
    // ---- fetch on mount + whenever the status filter changes ----
    // Inlined (rather than just calling loadPayments()) and guarded with a
    // `cancelled` flag so a slow, stale request can't overwrite a newer one
    // (e.g. user flips the status filter twice quickly).
    useEffect(() => {
        let cancelled = false;
    ...
    // React Compiler forbids mutating refs during render (see prior fix),
    // so an effect is the only compiler-safe place left for this. The
    // "setState in effect" rule still flags it — that's expected and safe
    // to suppress here since this effect only ever sets one small, local
    // piece of UI state and does no data fetching or side effects.
    useEffect(() => {
...
const handleCreate = async (e) => {
    ...
    try {
        ...
        // បន្ថែម patient_id ទៅក្នុង payload (ឧទាហរណ៍៖ ទាញពី state ឬ form)
        const payload = {
            ...form,
            patient_id: form.patient_id || 1, // បញ្ចូល patient_id ដែលត្រូវគ្នានៅទីនេះ
        };
                ...
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => !saving && setShowAddModal(false)}>
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleCreate} className="px-5 py-4 space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Appointment ID</label>
                                <input
                                    type="text"
                                    required
